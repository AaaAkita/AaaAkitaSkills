...
- **统一接入卡口**：实施无感拦截策略，创建 `middleware/auth.global.ts`。
  - ⚠️ 注意：拦截未登录时若放行 SSO 回调路由（如 `?code=xxxx`），执行 `navigateTo('/login')` 等重定向时务必透传原始所有 `query` 参数，否则会导致鉴权凭证丢失！
- **入口页面**：提供附带 SSO 唤起 popup 和本地登录备用的登录承载页，放置于 `pages/login.vue`。
  - ⚠️ 注意：禁止在前端实现任何“如果得不到 Token 就强行读取本地 `JSON` 作为 Fallback”的冗余代码，这会导致线上全员使用同一账号发生覆写灾难。跨域部署时，**必须**确保项目所在域名和 SSO 提供商域名存在同一顶级域名关系或正确的反向代理。
